...
library(rethinking)
sf_bronx <- sf_bronx[is.na(sf_bronx$race) == FALSE & 
                       is.na(sf_bronx$sex) == FALSE & 
                       is.na(sf_bronx$build) == FALSE & 
                       is.na(sf_bronx$time) == FALSE, ]



##::::::::::::::::: Create Bronx dataset:
...
## Model will include the following variables:  
model.geo <- alist(
  Force ~ dbinom(1, p),
  logit(p) <- a_sub[Precinct] + 
    b_aian[Precinct]* AIAN + 
    b_aapi[Precinct] * AAPI + 
    b_black[Precinct] * Black + 
    b_hisp[Precinct] * Hispanic + 
    b_gender[Precinct] * Female + 
    b_medium[Precinct] * Medium + 
    b_muscular[Precinct] * Muscular + 
    b_thin[Precinct] * Thin + 
    b_time[Precinct] * Time, ## we would need a b_sub term for every predictor
  c(a_sub,
    b_aian,
    b_aapi,
    b_black,
    ...
    dmvnorm2(c(a,
               ...
               btime),
             sigma_sub,
             Rho), ## this needs to include a term for each of the model predictors
  a ~ dnorm(0, 2),
  baian ~ dnorm(0, 3),
  baapi ~ dnorm(0, 3),
  bblack ~ dnorm(0, 3),
  ...
  bmuscular ~ dnorm(0, 3),
  bthin ~ dnorm(0, 3),
  btime ~ dnorm(0, 3), ##needs to be one of these specifications for each of the terms; we can play with priors
