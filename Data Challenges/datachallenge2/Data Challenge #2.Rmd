...
###finding! It tells you whether a question is worth pursuing further or not.
  ...
  * Which linear model best predicts whether or not people will be detained using 
  all weapons (defined as `clips.seized`, `cartridge.sezied`, `small.arms.seized`,
  `long.guns.seized`) seized and organized crime as predictors?
...
###and explain why you did that
  ...
  * Create a new variable named `detained_resp` for to record whether people 
  where detained (1) or not (0) in order to do a response prediction of whether
...
p_liner_table <- table(testing$detained_resp, as.integer(p_linear > 0.5))
 
p_quadratic <- predict(quadratic, newdata = testing, type = "response")
p_quadratic_table <- table(testing$detained_resp, as.integer(p_quadratic > 0.5))

p_cubic <- predict(cubic, newdata = testing, type = "response")
p_cubic_table <- table(testing$detained_resp, as.integer(p_cubic > 0.5))

p_quartic <- predict(quartic, newdata = testing, type = "response")
p_quartic_table <- table(testing$detained_resp, as.integer(p_quartic > 0.5))
```

###1c	be explicit about the limitations of your anaylisis, due to estimation 
###or to the data itself
  * talk through the plot functions and the limitations associated with each one
