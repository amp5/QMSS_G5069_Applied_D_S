...
```{r}
AllData$army <- as.integer(AllData$army)
AllData$navy <- as.integer(AllData$navy)
AllData$other <- as.integer(AllData$other)
AllData$federal.police <- as.integer(AllData$federal.police)
AllData$state.police <- as.integer(AllData$state.police)
AllData$municipal.police <- as.integer(AllData$municipal.police)
AllData$ministerial.police <- as.integer(AllData$ministerial.police)
AllData$afi <- as.integer(AllData$afi)


AllData$military <- cbind(AllData$army, AllData$navy)

AllData$all_others <- cbind(AllData$other, 
...
# Exclude rows where both AllData$military == 1 & AllData$all_others == 1 because
# we need no overlap for a true comparison:
summary(AllData$military == 1, AllData$all_others == 1) # if true, then both groups participated in one event
summary(AllData$military == 0, AllData$all_others == 0) # if true, then neither group participated in one event - so who participated?  this should not have any hits
