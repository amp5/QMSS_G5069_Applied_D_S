...
###and explain why you did that
  ...
  * A new variable wil be created named `detained_resp` to record whether people 
...
```
  * Analysis of the output:  the quartic regression model had the highest 
  prediction accuracy of 0.785 or 78.5% in using wounded organized crime members
  and all weapons seized to predict whether people would be detained or not.  The
  quartic confusion matrix shows that the model correctly predicted that no one
  would be detained 2,086 times and people would be detained 32 times. Although 
  the model has the highest prediction accuracy, it is mainly driven by correct 
  predictions of no one being detained.  It does not do a successful job of 
  predicting when people will be detained.  Therefore, this model will not help
  Mexican authorities understand driven of detention rates, since it does not
  predict how the number of organized crime members wounded and weapons seized
  relate to whether people are detained or not. Furhermore, the model incorrectly
  predicted that people would be detained in 547 events when no one was detained.
  It also incorrectly predicted that people would not be detained when they were
  detained in 33 events.

###1c	be explicit about the limitations of your anaylisis, due to estimation 
###or to the data itself
  * The limitations of the regression model is shown through various diagnostic 
  plots for each model using the entire dataset ( models are in order from worst 
  to best prediction accuracy):
...
```  
  ...
  1.) Residuals vs Fitted plot (cubic regression): 
  
  2.) Normal Q-Q plot (cubic regression):
  
  3.) Scale-Location plot (cubic regression): 
  
  4.) Residuals vs Leverage plot (cubic regression): 
  
```{r}
plot(lm(detained_resp ~ poly(weapons_seized + organized.crime.wounded, degree = 2),
        data = AllData)) # quadratic regression
```
  
  1.) Residuals vs Fitted plot (quadratic regression): 
  
  2.) Normal Q-Q plot (quadratic regression):
  
  3.) Scale-Location plot (quadratic regression): 
  
  4.) Residuals vs Leverage plot (quadratic regression): 
  
```{r}
plot(lm(detained_resp ~ weapons_seized + organized.crime.wounded, 
        data = AllData))# linear regression 
```

  1.) Residuals vs Fitted plot (linear regression): 
  
  2.) Normal Q-Q plot (linear regression):
  
  3.) Scale-Location plot (linear regression): 
  
  4.) Residuals vs Leverage plot (linear regression): 
  
```{r}
plot(lm(detained_resp ~ poly(weapons_seized + organized.crime.wounded, degree = 4),
        data = AllData)) # quartic regression
```
 
  1.) Residuals vs Fitted plot (quartic regression): 
  
  2.) Normal Q-Q plot (quartic regression):
  
  3.) Scale-Location plot (quartic regression): 
  
  4.) Residuals vs Leverage plot (quartic regression): 
...
###your finding
  * refer to the plot functions?  or 
  make hstogram of x = weapons, x = organized crime wounded, y = WHETHER people were detained?
