...
AllData$both_groups <- ifelse(AllData$military > 0 & AllData$all_others > 0,
                              "yes",
                              "no") ## both AllData$military and AllData$all_others participated in the event

# Exclude rows where both AllData$military and AllData$all_others participated 
# because there should be no overlap for a true comparison in the model:
to_exclude <- which(AllData$both_groups == "yes")
to_exclude
AllData_excluded <- AllData[!c(to_exclude), ]
