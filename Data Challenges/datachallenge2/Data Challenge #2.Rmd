...
###finding! It tells you whether a question is worth pursuing further or not.
  ...
  * Are all weapons (defined as `clips.seized`, `cartridge.sezied`, `small.arms.seized`,
  `long.guns.seized`) seized and organized crime wounded good predictors for whether 
...
table(testing$detained_resp, as.integer(p_linear > 0.5))
 
p_quadratic <- predict(quadratic, newdata = testing, type = "response")
table(testing$detained_resp, as.integer(p_quadratic > 0.5))

p_cubic <- predict(cubic, newdata = testing, type = "response")
table(testing$detained_resp, as.integer(p_cubic > 0.5))

p_quartic <- predict(quartic, newdata = testing, type = "response")
table(testing$detained_resp, as.integer(p_quartic > 0.5))
```
