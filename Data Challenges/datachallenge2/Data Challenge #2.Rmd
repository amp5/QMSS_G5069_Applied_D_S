...
```{r}
AllData$military <- transform(AllData$army + AllData$navy) ## if == 2, both the `army` and `navy` participated in the event, of == 1 only one partiicpated in the event
...
AllData$all_others <- transform(AllData$other + 
                            ...
                            AllData$afi) ## if > 1 more than one group partiicpated in the event
...
which(AllData$military == 1, AllData$all_others == 1) # rows when both groups participated in one event
...
which(AllData$military == 0, AllData$all_others == 0)
