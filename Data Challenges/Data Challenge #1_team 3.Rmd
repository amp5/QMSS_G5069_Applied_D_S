...
fullData <- 
    Confrontations %>% 
        ...
        rename(day.orig = DIA, 
               month.orig = MES, 
               #year.orig = AÑO,#had to change this part
...
###1a) Provide a visualization that presents this information neatly.
  ...
  * Not applicable - see response is #1 above.
```{r}
#Brandon: 

#Dont run until you run 1B
b <- ggplot(fullData)
...
```{r}
#Stephanie - Brandon - delete/keep whatever you want here:
#Figure 1 - 86.1% of dead civilians who presumably participated in confrontations
#with federal armed forces (army and navy) were killed in events of "perfect 
#lethality" where there were only dead and no wounded:

#none of this is correct because we don't have the number of total civilians who were niether killed nor wounded:

sum(fullData$civilian.dead) / sum(fullData$civilian.wounded)
((sum(fullData$civilian.dead) / sum(fullData$total.people.dead)) * 100)
((sum(fullData$civilian.wounded) / sum(fullData$total.people.wounded)) * 100)
sum(fullData$total.people.dead) / sum(fullData$total.people.wounded) * 100

# number of dead civilians who presumably participated in confrontations
#with federal armed forces (army and navy):
new_data3 <- subset(fullData,
                     civilian.dead > 0 &
                       civilian.wounded == 0 &
                      military.wounded == 0 &
                      navy.wounded == 0) #total number of civilians

sum(new_data3$civilian.dead) / sum(new_data3$total.people.wounded)

#Overall lethality ratio:
#none of this is correct:
sum(fullData$total.people.dead) / sum(fullData$total.people.wounded)

#Federal Police:
#none of this is correct:
sum(fullData$federal.police.dead) / sum(fullData$federal.police.wounded) 
sum(fullData$federal.police.dead) / sum(fullData$total.people.wounded) * 100
new_data <- subset(fullData,
                     military.dead == 0 &
                     navy.dead == 0 &
                     federal.police.dead &
                     afi.dead == 0 &
                     state.police.dead == 0 &
                     ministerial.police.dead == 0 &
                     municipal.police.dead == 0 &
                     public.prosecutor.dead == 0 &
                     organized.crime.dead &
                     civilian.dead > 0)
sum(new_data$federal.police.dead) / sum(new_data$federal.police.wounded)
sum(new_data$civilian.dead) / sum(new_data$civilian.wounded)
sum(new_data$federal.police.dead, new_data$civilian.dead) / sum(new_data$federal.police.wounded, new_data$civilian.wounded)

new_data2 <- subset(fullData,
                   military.dead == 0 &
                     navy.dead == 0 &
                     federal.police.dead &
                     afi.dead == 0 &
                   ...
                   military.wounded == 0 &
                     navy.wounded == 0 &
                     federal.police.wounded &
                     afi.wounded == 0 &
                     state.police.wounded == 0 &
                     ministerial.police.wounded == 0 &
                     municipal.police.wounded == 0 &
                     public.prosecutor.wounded == 0 &
                     organized.crime.wounded &
                     civilian.wounded) #doesn't result in anything useful. 

#Navy:
sum(fullData$navy.dead) / sum(fullData$navy.wounded) #incorrect

#Army:
sum(fullData$military.dead) / sum(fullData$military.wounded) #incorrect
...
###1d) Be very explicit: What are you assuming to generate these computations?
  ...
  * The assumption is that all civilian interactions are either civilians who were
  ...
  each observation (civilian.dead > 0 and civilian.wounded > 0) is also associated 
  with a wounded or dead observation for the federal police, military, and navy (e.g.,
  navy.dead > 0 and/or navy.wounded > 0).
  
  We have no infomation on those who were not wounded or dead and these individuals may 
  have been involved but were able to escape without a scratch. This means according to 
  our calculations we are assuming these individuals are nonexistant, this may be problematic.

  Also we do not have information on whether the civilians involved were wounded/dead 
  because of the Government forces or from the organized criminals, this information not
  being there means we are just assuming all civilians injured or dead was caused by the 
  Goverment agencies.
...
definition of confrontation, there must “be at least three (3) organized crime 
members, or less if using military-grade equipment and explosives,” which indicates 
...
restraints, such as the instance must involve “violent resistance to armed forces 
and other government authorities” which would preclude an instance from being 
...
confrontation has occurred. The article does use the word “confrontation” 
...
reading “confrontation” as presented in the piece. Therefore, the presentation of 
...
situations – therefore undermining the claim of differential training that the piece
suggests. 
...
Therefore, this index is not only censoring, but also reductive by definition – 
...
quelling targets without killing civilians (or wounding without killing) – which the
...
– especially if the civilian count is 0 in a given confrontation, which is 
...
Specifically – in order to make the claim that police officers in Mexico are good or
...
there is not necessarily an absolute “best” way to measure the lethality index, 
rather it is critical to define the index in the context of the insight that the 
analyst wishes to draw and then evaluate the data’s ability to do so. Rather, it is 
not necessarily fully possible to construct a “better” measure of the lethality 
